...
然后就是大概过了一个月又回来看，发现了一些小问题，不过我觉得对于这个Dataset而言也没有什么比较好细究的了。情感二分类有非常多样的作弊方式，并不有很强的说服力。

大概想到以下的几点吧：

- Embedding效果也会显著影响NLP系统的效果，而上面的每个模型batch都是各自学习的embedding，所以不是很好得出“上下文而不是模型架构的区别”，如果想做ablation，可能冻结一个词库然后改变架构会更严谨一点
- 如果能够把这个实验升级，不光判断情感态度，而是给出星级的监督学习，可能效果会更好。但是本人显然没有这么大的力气一一的去标。虽然可以采用GPT去标，但是AI生成Label再训练的伪监督学习其实不就相当于蒸馏了一部分模型吗（虽然也算是业内的标配
- 里面好像没有涉及比较多的工程知识，之后会补习一下